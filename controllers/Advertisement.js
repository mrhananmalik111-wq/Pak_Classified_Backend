...
async function Advertisement(req, res) {
    try {
        const { Name, Description, EndsOn, Features, Price, StartsOn, CityArea, Category, userId, Image } = req.body;
        
        // ✅ YEH SAHI HAI - req.body se Image URL check karo
        if (!Image) {
            return res.status(400).json({ message: "Please upload an image." });
        }

        // Required fields check
        if (!Name || !Description || !EndsOn || !Features || !Price || !StartsOn || !CityArea || !Category) {
        ...
        const Data = await Advertisement_Model.create({
            ...
            Image: Image, // ✅ Frontend se aayi hui Cloudinary URL
